...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelector('#bt-salvar').addEventListener('click', function () {
        ...
        let fidelidadeEl = document.querySelector('input[name="campo-fidelidade"]:checked');
        let fidelidade = fidelidadeEl ? fidelidadeEl.value : "";
        let observacao = document.querySelector('#campo-05') ? document.querySelector('#campo-05').value : "";

        // Validações de campos
        if (nomeCompleto == "") {
            alert("O nome completo do cliente é um campo obrigatório!");
            return;
        } else if (dataNascimento == "") {
            alert("A data de nascimento do cliente é um campo obrigatório!");
            return;
        } else if (documento == "") {
            alert("Documento é um campo obrigatório!");
            return;
        }   else if (fidelidade == "") {
            alert("Fidelidade é um campo obrigatório!");
            return;
        }
...
function listar() {
    ...
    listaPessoas.forEach(function (objeto) {
        ...
        htmlColunas += "<td>" + (objeto.fidelidade ? objeto.fidelidade : "") + "</td>";
        htmlColunas += "<td>" + (objeto.observacao ? objeto.observacao : "") + "</td>";
...
function eventosListagem() {
    ...
    document.querySelectorAll('.bt-editar').forEach(function (botao) {
        botao.addEventListener('click', function () {
            ...
            let fidelidade = colunas[7] ? colunas[7].textContent : "";
            let observacao = colunas[8] ? colunas[8].textContent : "";
            ...
            document.querySelectorAll('input[name="campo-fidelidade"]').forEach(r => r.checked = (r.value === fidelidade));
            if (document.querySelector('#campo-05')) {
                document.querySelector('#campo-05').value = observacao;
            }
...
function resetarForm() {
    ...
    document.querySelectorAll('input[name="campo-fidelidade"]').forEach(r => r.checked = false);
    if (document.querySelector('#campo-05')) {
